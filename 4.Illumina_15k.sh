#Step 4 to prepare the Illumina 15k array for imputation. This section requires several external script - read below for requirements.
...
plink --bfile illumina.15.QC9 --indep-pairwise 1000 5 0.2 --out illumina.15.QC9
plink --bfile illumina.15.QC9 --extract illumina.15.QC9.prune.in --make-bed --pca header tabs --out illumina.15.pca

#Genetate allele frequencies from clean file.
plink --bfile illumina.15.QC9 --freq --out illumina.15

#Now check data using Will Rayner's checking script. Script and associated files are available here: 
#	https://www.well.ox.ac.uk/~wrayner/tools/
#Run using code:
perl HRC-1000G-check-bim.pl -b illumina.15.QC9.bim -f illumina.15.frq -r HRC.r1-1.GRCh37.wgs.mac5.sites.tab -h
...
for CHR in {1..23};
do
plink --bfile illumina.15.QC9-updated-chr${CHR} --recode vcf --out illumina.15.updated.chr${CHR}
bcftools sort illumina.15.updated.chr${CHR}.vcf -O z -o illumina.15.chr${CHR}.vcf.gz
done
