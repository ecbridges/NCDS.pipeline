...
R 
library(plinkQC) 
exclude_ancestry.infin.1 <-  evaluate_check_ancestry(indir="", name="",
                          prefixMergedDataset=,
                          europeanTh = 3,
...
write.table(file = "infin.1.ancestry.txt", exclude_ancestry.infin.1$fail_ancestry, 
	    row.names = F, quote = F)

#Remove ancestry outliers.
plink --bfile infin.1.QC8 --remove infin.1.ancestry.txt --make-bed --out infin.1.QC9
...
plink --bfile infin.1.QC9 --indep-pairwise 1000 5 0.2 --out infin.1.QC9
plink --bfile infin.1.QC9 --extract infin.1.QC9.prune.in --make-bed --pca header tabs --out infin.1.pca

#Genetate allele frequencies from clean file.
plink --bfile infin.1.QC9 --freq --out infin.1

#Now check data using Will Rayner's checking script. Script and associated files are available here: 
#	https://www.well.ox.ac.uk/~wrayner/tools/
#Run using code:
perl HRC-1000G-check-bim.pl -b infin.1.QC9.bim -f infin.1.frq -r HRC.r1-1.GRCh37.wgs.mac5.sites.tab -h
...
for CHR in {1..23};
do
plink --bfile infin.1.QC9-updated-chr${CHR} --recode vcf --out infin.1.updated.chr${CHR}
