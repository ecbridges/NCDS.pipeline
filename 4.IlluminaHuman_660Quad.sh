...
plink --bfile quad.QC8 --indep-pairwise 1000 5 0.2 --out quad.QC8
plink --bfile quad.QC8 --extract quad.QC8.prune.in --make-bed --pca header tabs --out quad.pca

#Genetate allele frequencies from clean file.
plink --bfile quad.QC8 --freq --out quad

#Now check data using Will Rayner's checking script. Script and associated files are available here: 
#	https://www.well.ox.ac.uk/~wrayner/tools/
#Run using code:
perl HRC-1000G-check-bim.pl -b quad.QC8.bim -f quad.frq -r HRC.r1-1.GRCh37.wgs.mac5.sites.tab -h
...
for CHR in {1..23};
do
plink --bfile quad.QC8-updated-chr${CHR} --recode vcf --out quad.updated.chr${CHR}
