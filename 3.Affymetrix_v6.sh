...
plink --bfile affy.6.QC5 --set-hh-missing --make-bed affy.6.QC6

#Sex check on pruned dataset.
plink --bfile affy.6.QC6 --indep-pairwise 1000 5 0.2 --out affy.6.QC6
plink --bfile affy.6.QC6 --extract affy.6.QC6.prune.in --sex-check --out affy.6.sex
grep PROBLEM affy.6.sex.sexcheck >> affy.6.sex.probs.txt

#Remove individuals with sex errors.
plink --brfile affy.6.QC6 --remove affy.6.sex.probs.txt --make-bed --out affy.6.QC7

#Check for related individuals.
plink --bfile affy.6.QC7 --indep-pairwise 1500 150 0.2 --out affy.6.QC7
plink --bfile affy.6.QC7 --extract affy.6.prune.in --make-bed --out pruned.affy.6.QC7
plink --bfile pruned.affy.6.QC7 --genome --out affy.6
...
plink --bfile affy.6.QC7 --remove affy.6.related.excl.txt --make-bed --out affy.6.QC8
